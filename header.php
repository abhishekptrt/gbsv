<div class="container-fluid">
        <div class="row">
            <div class="col-lg-2  d-none d-lg-block" style="margin: 0;padding: 0;">
                <a href="index.html" class="navbar-brand w-100 h-100 m-0 p-0 d-flex align-items-center justify-content-center">
                   <!-- <h1 class="m-0 display-3 text-primary">Klean</h1>-->
                    <img src="img/logo4.jpg">
                </a>
            </div>
            <div class="col-lg-10">
                <div class="row bg-dark d-none d-lg-flex tp-class">
                    <div class="col-lg-12 text-left text-white">
                        <div class=" py-2 px-3">
                           <h4>श्री गुमान बिहारी संस्कृत माध्यमिक विद्यालय..(चरखारी महोबा) </h4><br>
                            <p>उत्तर प्रदेश माध्यमिक शिक्षा परिषद लखनऊ से सम्बद्ध</p>
                        </div>
                    </div>
                    
                </div>
                <nav class="navbar navbar-expand-lg bg-white navbar-light p-0">
                    <a href="index.php" class="navbar-brand d-block d-lg-none">
                        <img src="img/logo4.jpg">
                    </a>
                    <button type="button" class="navbar-toggler" data-toggle="collapse" data-target="#navbarCollapse">
                        <span class="navbar-toggler-icon"></span>
                    </button>
                   <div class="collapse navbar-collapse justify-content-between" id="navbarCollapse">
                        <div class="navbar-nav mr-auto py-0">
                            <a href="index.php" class="nav-item nav-link active">Home</a>
                            <a href="about.php" class="nav-item nav-link">About</a>
                            <a href="teachers.php" class="nav-item nav-link">Teachers</a>
                             <a href="gallery.php" class="nav-item nav-link">Gallery</a>
                            <a href="contact.php" class="nav-item nav-link">Contact</a>
                        </div>
                       <!-- <a href="" class="btn btn-primary mr-3 d-none d-lg-block">Get A Quote</a>-->
                    </div>
                </nav>
                
                
            </div>
        </div>
    </div>